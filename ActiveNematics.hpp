...
namespace ActiveNematics_2D{
  ...
  vector<Particle> PolarParticle2D_GT_To_vAN(PolarParticle2D_GT PG){
  ...
  void UpdatePGFromVAN(PolarParticle2D_GT& PG,vector<Particle> vAN)
  {//Update a PolarParticle2D_GT based on a vector<Particle>, only Pos and Dir changes
    for( int i=0 ; i<PG.size() ; i++ ){
      PG[i].Pos=vAN[i].x+vAN[i].y*ii;
      PG[i].Dir=atan2(vAN[i].vy,vAN[i].vx);
  ...
  void Evolve_AN_Turned1FoldMean(vector<Particle>& PG,
				 vector<vector<int>>& MeshedIndexMat,
				 const vector<array<int,4>>& LDAdjBox,
				 const vector<double>& Parameters,// Systemsize_x Systemsize_y Charlength Velocity Sigma Alpha (Guidingcoeff)
  ...
  {// Dir(t+1) = (\pm 1) * arg( mean( sgn( cos( NeighborDir - Dir ) ) * exp( i NeighborDir ) ) )
   // Each step the vector have a probability Alpha of reversing (+1 to -1)
    double
      ...
      Alpha=Parameters[5],
      GuidingCoeff=0;
    if(Parameters.size()==7) GuidingCoeff=Parameters[6];
	...
	cerr<<"Attention: zero vector sum causing dir decision failure."<<endl;
      ...
      //probable flipping
      if(RandGen[i_MP].RandomDouble()<=Alpha)
	NewDir+=pi;
  ...
  PolarParticle2D_GT Evolve_AN_Turned1FoldMean_Periodic(const PolarParticle2D_GT& PG,
							  const BoxInfoT& BoxInfo,
							  const vector<double>& Parameters,// Systemsize_x Systemsize_y Charlength Velocity Sigma Alpha Guidingcoeff
							  const int& StepNumber,
							  const int& i_MP=0)
  {//an integrated version of Evolve_AN_Turned1FoldMean, evolving for StepNumber steps
    const double
      SystemSize_X=Parameters[0],
      SystemSize_Y=Parameters[1],
      CharLength=Parameters[2];
    
    vector<Particle> vAN=PolarParticle2D_GT_To_vAN(PG);
    ...
    for( int i_Step=0 ; i_Step<StepNumber ; i_Step++ )
      Evolve_AN_Turned1FoldMean(vAN,MeshedIndex,LDAdjBox,Parameters,Distance_,i_MP);
    
    auto ansPG=PG;
    UpdatePGFromVAN(ansPG,vAN);
    return ansPG;
  }

#ifndef LocalProgram

  void Evolve_AN_Turned1FoldMean_MP(vector<Particle>& PG,
				    vector<vector<int>>& MeshedIndexMat,
  ...
  {// Dir(t+1) = (\pm 1) * arg( mean( sgn( cos( NeighborDir - Dir ) ) * exp( i NeighborDir ) ) )
	...
	cerr<<"Attention: zero vector sum causing dir decision failure."<<endl;
      ...
      //probable flipping
      if(RandGen[omp_get_thread_num()].RandomDouble()<=Alpha)
	NewDir+=pi;
  ...
  PolarParticle2D_GT Evolve_AN_Turned1FoldMean_Periodic_MP(const PolarParticle2D_GT& PG,
							   const BoxInfoT& BoxInfo,
							   const vector<double>& Parameters,// Systemsize_x Systemsize_y Charlength Velocity Sigma Alpha Guidingcoeff
							   const int& StepNumber)
  {//an integrated version of Evolve_AN_Turned1FoldMean, evolving for StepNumber steps
    ...
    for( int i_Step=0 ; i_Step<StepNumber ; i_Step++ )
      Evolve_AN_Turned1FoldMean_MP(vAN,MeshedIndex,AllAdjBox,Parameters,Distance_);
